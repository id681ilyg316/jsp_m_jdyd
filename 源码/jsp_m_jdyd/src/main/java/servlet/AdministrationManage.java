...
@WebServlet("/AdministrationManage")
public class AdministrationManage extends HttpServlet{
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if(mop==4){
            response.sendRedirect(request.getContextPath()+"/systemManagement/waiterAdd.jsp?mop=4");
...
//            System.out.println("有么有么，成功了么？？？"+waiter);
//            Query.insertWaiter(waiter);
        }else if(mop==5) {
            response.sendRedirect(request.getContextPath()+"/systemManagement/waiterEdit.jsp?mop=5");
        }else if(mop==6){
            response.sendRedirect(request.getContextPath()+"/systemManagement/waiterUpdate.jsp?mop=6");
        }else if(mop==7){
            response.sendRedirect(request.getContextPath()+"/systemManagement/waiterShow.jsp?mop=7");
        } else if (mop == 10) {//业务数据统计
            response.sendRedirect(request.getContextPath()+"/systemManagement/statistics.jsp?mop=10");
        }
